...
export async function getServerSideProps(context) {
  const postsQuery = firestore
    .collectionGroup('posts') // grab any sub collection no matter where exist in document
...
export default function Home(props) {
  ...
  const getMorePosts = async () => {
    setLoading(true);
    const last = posts[posts.length - 1]; // last post in the current list
    ...
    if (newPosts.length < LIMIT) {
      setPostsEnd(true); // reach the end of db
    }
  };
